...
//! Implementation of the CodeParser trait for Elm
...
/// Elm language parser implementing the CodeParser trait
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = ElmParser::new();
        assert_eq!(parser.file_extensions(), &[".elm"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = ElmParser::new();
        assert!(parser.can_parse(Path::new("Main.elm")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Elm module touching every extracted
    /// entity kind: one import, one type declaration (mapped to a class), and one
    /// top-level value declaration (mapped to a function). Elm's IR never carries
    /// traits, so the trait count stays at zero. `visit_node` only walks the
    /// module's direct children, so a lone type annotation plus its definition
    /// collapse to a single function.
    const SAMPLE: &str = "module Sample exposing (main)\n\nimport Html exposing (Html)\n\ntype Msg = Increment | Decrement\n\nmain : Int\nmain =\n    42\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = ElmParser::default().metrics();
        let new = ElmParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = ElmParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("Sample.elm"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one top-level value declaration");
        assert_eq!(info.classes.len(), 1, "type declaration maps to a class");
        assert_eq!(info.traits.len(), 0, "elm never produces traits");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Elm is tree-sitter-based and error-tolerant, so a module header with
        // only a comment parses cleanly and simply extracts nothing.
        let parser = ElmParser::new();
        let mut g = graph();
        let src = "module Empty exposing (..)\n\n-- just a comment\n";
        let info = parser
            .parse_source(src, Path::new("Empty.elm"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "Sample.elm", SAMPLE);
        let parser = ElmParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.elm"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.elm", SAMPLE);
        let parser = ElmParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "A.elm", SAMPLE);
        let b = write_file(dir.path(), "B.elm", SAMPLE);
        let parser = ElmParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "A.elm", SAMPLE);
        let b = write_file(dir.path(), "B.elm", SAMPLE);
        let parser = ElmParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "Good.elm", SAMPLE);
