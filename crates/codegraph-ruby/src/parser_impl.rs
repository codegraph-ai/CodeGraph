...
mod tests {
    ...
    /// A small but syntactically complete Ruby source touching every extracted
    /// entity kind: one require (import), one module (trait), one class with one
    /// method (function). Ruby maps modules to traits and both class and module
    /// methods to functions; this sample keeps the module method-free so the
    /// function count is exactly one.
    ...
    fn test_default_matches_new_metrics() {
        let default = RubyParser::default().metrics();
        let new = RubyParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = RubyParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("lib.rb"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one class method");
        assert_eq!(info.classes.len(), 1, "one class");
        assert_eq!(info.traits.len(), 1, "module maps to a trait");
        assert_eq!(info.imports.len(), 1, "one require");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        ...
        let info = parser
            .parse_source(src, Path::new("empty.rb"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.rb", SAMPLE);
        let parser = RubyParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.rb"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.rb", SAMPLE);
        let parser = RubyParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.rb", SAMPLE);
        let b = write_file(dir.path(), "b.rb", SAMPLE);
        let parser = RubyParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.rb", SAMPLE);
        let b = write_file(dir.path(), "b.rb", SAMPLE);
        let parser = RubyParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.rb", SAMPLE);
