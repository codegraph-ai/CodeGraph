...
//! Implementation of the CodeParser trait for Objective-C
...
/// Objective-C language parser implementing the CodeParser trait
...
impl CodeParser for ObjcParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = ObjcParser::new();
        assert_eq!(parser.file_extensions(), &[".m", ".mm"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = ObjcParser::new();
        assert!(parser.can_parse(Path::new("MyClass.m")));
        assert!(parser.can_parse(Path::new("MyClass.mm")));
    ...
    /// A small but syntactically complete Objective-C source touching every
    /// extracted entity kind: one `#import` (import), one `@protocol` (trait),
    /// one `@interface` (class), and one `@implementation` method definition
    /// (function). The protocol and interface carry no method declarations so
    /// the visitor cannot inflate the function count, and only the
    /// implementation's single method definition becomes a function; this pins
    /// functions=1/classes=1/traits=1/imports=1 with entity_count=3.
    const SAMPLE: &str = r#"#import "MyHelper.h"
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = ObjcParser::default().metrics();
        let new = ObjcParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = ObjcParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("MyClass.m"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one implementation method");
        assert_eq!(info.classes.len(), 1, "@interface maps to a class");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Objective-C is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = ObjcParser::new();
        let mut g = graph();
        let src = "// just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.m"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "MyClass.m", SAMPLE);
        let parser = ObjcParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.m"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.m", SAMPLE);
        let parser = ObjcParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.m", SAMPLE);
        let b = write_file(dir.path(), "b.m", SAMPLE);
        let parser = ObjcParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.m", SAMPLE);
        let b = write_file(dir.path(), "b.m", SAMPLE);
        let parser = ObjcParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.m", SAMPLE);
