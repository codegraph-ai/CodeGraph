...
mod tests {
    ...
    /// A small but syntactically complete Java source touching every extracted
    /// entity kind: one import, one interface (trait), one class implementing
    /// it, and two methods (the interface's `greet` signature plus the class's
    /// `greet` body) which both count as functions.
    const SAMPLE: &str = "package com.example;\n\nimport java.util.List;\n\npublic interface Greeter {\n    String greet();\n}\n\npublic class HelloWorld implements Greeter {\n    public String greet() {\n        return \"hi\";\n    }\n}\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = JavaParser::default().metrics();
        let new = JavaParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = JavaParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("HelloWorld.java"), &mut g)
            .expect("parse ok");
        assert_eq!(
            info.functions.len(),
            2,
            "interface signature + class method"
        );
    ...
    fn test_parse_source_minimal_yields_no_entities() {
        ...
        let info = parser
            .parse_source(src, Path::new("Empty.java"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "HelloWorld.java", SAMPLE);
        let parser = JavaParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/File.java"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "Big.java", SAMPLE);
        let parser = JavaParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "A.java", SAMPLE);
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "A.java", SAMPLE);
        let b = write_file(dir.path(), "B.java", SAMPLE);
        let parser = JavaParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "Good.java", SAMPLE);
