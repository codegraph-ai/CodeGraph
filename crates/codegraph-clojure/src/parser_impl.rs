...
mod tests {
    ...
    /// A small, complete Clojure namespace touching each entity kind Clojure
    /// supports. The `(ns ... (:require ...))` clause yields one import
    /// (clojure.string), `defrecord` yields one class (Point, with empty
    /// methods), and `defn` yields one function (greet). Clojure has no trait
    /// concept and the mapper always leaves trait_ids empty, so this pins
    /// functions=1 / classes=1 / traits=0 / imports=1 with entity_count=2
    /// (functions + classes + traits).
    const SAMPLE: &str = r#"(ns my.app
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = ClojureParser::default().metrics();
        let new = ClojureParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = ClojureParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("app.clj"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one defn");
        assert_eq!(info.classes.len(), 1, "one defrecord");
        assert_eq!(info.traits.len(), 0, "Clojure has no traits");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Clojure is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts no entities.
        let parser = ClojureParser::new();
        let mut g = graph();
        let src = "; just a comment\n";
        let info = parser
            .parse_source(src, Path::new("app.clj"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "app.clj", SAMPLE);
        let parser = ClojureParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/app.clj"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "app.clj", SAMPLE);
        let parser = ClojureParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_reset_metrics_zeroes_state() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "app.clj", SAMPLE);
        let mut parser = ClojureParser::new();
        let mut g = graph();
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.clj", SAMPLE);
        let b = write_file(dir.path(), "b.clj", SAMPLE);
        let parser = ClojureParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.clj", SAMPLE);
        let b = write_file(dir.path(), "b.clj", SAMPLE);
        let parser = ClojureParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.clj", SAMPLE);
        let missing = dir.path().join("missing.clj");
        let parser = ClojureParser::new();
        let mut g = graph();
        let project = parser
