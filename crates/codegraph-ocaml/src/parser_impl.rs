...
//! Implementation of the CodeParser trait for OCaml
...
/// OCaml language parser implementing the CodeParser trait
...
impl CodeParser for OcamlParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = OcamlParser::new();
        assert_eq!(parser.file_extensions(), &[".ml", ".mli"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = OcamlParser::new();
        assert!(parser.can_parse(Path::new("main.ml")));
        assert!(parser.can_parse(Path::new("types.mli")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete OCaml module touching every extracted
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = OcamlParser::default().metrics();
        let new = OcamlParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = OcamlParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("greeter.ml"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one let binding function");
        assert_eq!(info.classes.len(), 0, "OCaml has no classes");
        assert_eq!(info.traits.len(), 0, "OCaml has no traits");
        assert_eq!(info.imports.len(), 1, "one open directive");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // OCaml is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = OcamlParser::new();
        let mut g = graph();
        let src = "(* just a comment *)\n";
        let info = parser
            .parse_source(src, Path::new("empty.ml"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "greeter.ml", SAMPLE);
        let parser = OcamlParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.ml"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.ml", SAMPLE);
        let parser = OcamlParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_reset_metrics_zeroes_state() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "greeter.ml", SAMPLE);
        let mut parser = OcamlParser::new();
        let mut g = graph();
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.ml", SAMPLE);
        let b = write_file(dir.path(), "b.ml", SAMPLE);
        let parser = OcamlParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.ml", SAMPLE);
        let b = write_file(dir.path(), "b.ml", SAMPLE);
        let parser = OcamlParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.ml", SAMPLE);
        let missing = dir.path().join("missing.ml");
        let parser = OcamlParser::new();
        let mut g = graph();
        let project = parser
