...
//! Implementation of the CodeParser trait for R
...
/// R language parser implementing the CodeParser trait
pub struct RParser {
    config: ParserConfig,
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = RParser::new();
        assert_eq!(parser.file_extensions(), &[".r", ".R", ".Rmd"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = RParser::new();
        assert!(parser.can_parse(Path::new("analysis.R")));
        assert!(parser.can_parse(Path::new("analysis.r")));
        assert!(parser.can_parse(Path::new("report.Rmd")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete R source touching every extracted
    /// entity kind. R's visitor carries only functions and imports (no classes,
    /// no traits): one `library(...)` call is an import and one
    /// `name <- function(...) {}` binding is a function. This pins
    /// functions=1/classes=0/traits=0/imports=1 with entity_count=1.
    const SAMPLE: &str = r#"library(ggplot2)
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = RParser::default().metrics();
        let new = RParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = RParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("analysis.R"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one function binding");
        assert_eq!(info.classes.len(), 0, "R extracts no classes");
        assert_eq!(info.traits.len(), 0, "R extracts no traits");
        assert_eq!(info.imports.len(), 1, "one library() import");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // R is tree-sitter-based and error-tolerant, so a comment-only source
        // parses cleanly and simply extracts nothing.
        let parser = RParser::new();
        let mut g = graph();
        let src = "# just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.R"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "analysis.R", SAMPLE);
        let parser = RParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.R"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.R", SAMPLE);
        let parser = RParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.R", SAMPLE);
        let b = write_file(dir.path(), "b.R", SAMPLE);
        let parser = RParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.R", SAMPLE);
        let b = write_file(dir.path(), "b.R", SAMPLE);
        let parser = RParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.R", SAMPLE);
        let missing = dir.path().join("missing.R");
        let parser = RParser::new();
        let mut g = graph();
        let project = parser
