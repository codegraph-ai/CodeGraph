...
//! Implementation of the CodeParser trait for Haskell
...
/// Haskell language parser implementing the CodeParser trait
...
impl CodeParser for HaskellParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = HaskellParser::new();
        assert_eq!(parser.file_extensions(), &[".hs"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = HaskellParser::new();
        assert!(parser.can_parse(Path::new("Main.hs")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Haskell module touching every
    /// extracted entity kind. Haskell's IR carries functions, classes, and
    /// imports but never traits: `greet` (signature + definition) is a single
    /// function, `data Color` maps to a class, and `import Data.Text` is one
    /// import. The signature is folded into the function, not counted
    /// separately, so this pins functions=1/classes=1/traits=0/imports=1.
    const SAMPLE: &str = "module Sample where\n\nimport Data.Text (Text)\n\ngreet :: String -> String\ngreet name = \"Hello, \" ++ name\n\ndata Color = Red | Green | Blue\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = HaskellParser::default().metrics();
        let new = HaskellParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = HaskellParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("Sample.hs"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "greet");
        assert_eq!(info.classes.len(), 1, "data Color");
        assert_eq!(info.traits.len(), 0, "Haskell has no traits in the IR");
        assert_eq!(info.imports.len(), 1, "import Data.Text");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Haskell is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = HaskellParser::new();
        let mut g = graph();
        let src = "-- just a comment\n";
        let info = parser
            .parse_source(src, Path::new("Empty.hs"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "Sample.hs", SAMPLE);
        let parser = HaskellParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.hs"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.hs", SAMPLE);
        let parser = HaskellParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "A.hs", SAMPLE);
        let b = write_file(dir.path(), "B.hs", SAMPLE);
        let parser = HaskellParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "A.hs", SAMPLE);
        let b = write_file(dir.path(), "B.hs", SAMPLE);
        let parser = HaskellParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "Good.hs", SAMPLE);
        let missing = dir.path().join("Missing.hs");
        let parser = HaskellParser::new();
        let mut g = graph();
        let project = parser
