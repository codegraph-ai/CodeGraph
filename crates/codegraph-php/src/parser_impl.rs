...
mod tests {
    ...
    /// A small but syntactically complete PHP source touching every extracted
    /// entity kind: one top-level `use` (import), one interface (trait), one
    /// class with one method (function). PHP maps both interfaces and traits to
    /// ir.traits and visits their method bodies into ir.functions, so the
    /// interface is kept method-free to make the function count exactly one.
    ...
    fn test_default_matches_new_metrics() {
        let default = PhpParser::default().metrics();
        let new = PhpParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = PhpParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("lib.php"), &mut g)
            .expect("parse ok");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        ...
        let src = "<?php\n// just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.php"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.php", SAMPLE);
        let parser = PhpParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.php"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.php", SAMPLE);
        let parser = PhpParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.php", SAMPLE);
        let b = write_file(dir.path(), "b.php", SAMPLE);
        let parser = PhpParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.php", SAMPLE);
        let b = write_file(dir.path(), "b.php", SAMPLE);
        let parser = PhpParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.php", SAMPLE);
