...
/// Check if a class name follows the controller convention (Laravel, Symfony).
fn is_php_controller_class(name: &str) -> bool {
    name.ends_with("Controller")
}

/// Skip PHP lifecycle/framework methods that aren't HTTP actions.
fn is_php_lifecycle_method(name: &str) -> bool {
    let lower = name.to_lowercase();
    lower.starts_with("__")  // __construct, __destruct, __call, etc.
        || lower == "boot"
        || lower == "register"
        || lower.starts_with("middleware")
