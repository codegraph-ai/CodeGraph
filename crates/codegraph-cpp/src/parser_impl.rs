...
mod tests {
    ...
    /// A small but syntactically complete C++ source touching the extracted
    /// entity kinds the visitor emits from plain source: one include, one class
    /// (a struct-like data holder, no methods), and one free function. C++
    /// traits are only produced for pure-abstract classes, so this SAMPLE keeps
    /// traits at zero and the function count at exactly one.
    const SAMPLE: &str = "#include <cstdint>\n\nclass Point {\npublic:\n    int x;\n};\n\nint add(int a, int b) {\n    return a + b;\n}\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = CppParser::default().metrics();
        let new = CppParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = CppParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("lib.cpp"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one free function");
        assert_eq!(info.classes.len(), 1, "one class");
        assert_eq!(info.traits.len(), 0, "no pure-abstract classes");
        assert_eq!(info.imports.len(), 1, "one include");
    ...
    fn test_parse_source_minimal_yields_no_entities() {
        ...
        let info = parser
            .parse_source(src, Path::new("empty.cpp"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.cpp", SAMPLE);
        let parser = CppParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.cpp"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.cpp", SAMPLE);
        let parser = CppParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.cpp", SAMPLE);
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.cpp", SAMPLE);
        let b = write_file(dir.path(), "b.cpp", SAMPLE);
        let parser = CppParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.cpp", SAMPLE);
