...
//! Implementation of the CodeParser trait for Julia
...
/// Julia language parser implementing the CodeParser trait
...
impl CodeParser for JuliaParser {
...
mod tests {
    ...
    fn test_language() {
        ...
        assert_eq!(parser.language(), "julia");
    }

    #[test]
    fn test_file_extensions() {
        let parser = JuliaParser::new();
        assert_eq!(parser.file_extensions(), &[".jl"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = JuliaParser::new();
        assert!(parser.can_parse(Path::new("script.jl")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Julia source touching every extracted
    /// entity kind: one `using` import, one `struct` (class), one `abstract type`
    /// (trait), one top-level function. The struct is field-only and the abstract
    /// type body-less so the visitor cannot inflate the function count; this pins
    /// functions=1/classes=1/traits=1/imports=1 with entity_count=3.
    const SAMPLE: &str = "using LinearAlgebra\n\nabstract type Shape end\n\nstruct Point\n    x::Int\n    y::Int\nend\n\nfunction greet(name)\n    println(name)\nend\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = JuliaParser::default().metrics();
        let new = JuliaParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = JuliaParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("lib.jl"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one top-level function");
        assert_eq!(info.classes.len(), 1, "struct maps to a class");
        assert_eq!(info.traits.len(), 1, "abstract type maps to a trait");
        assert_eq!(info.imports.len(), 1, "one using import");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Julia is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = JuliaParser::new();
        let mut g = graph();
        let src = "# just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.jl"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.jl", SAMPLE);
        let parser = JuliaParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.jl"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.jl", SAMPLE);
        let parser = JuliaParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_reset_metrics_zeroes_state() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "lib.jl", SAMPLE);
        let mut parser = JuliaParser::new();
        let mut g = graph();
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.jl", SAMPLE);
        let b = write_file(dir.path(), "b.jl", SAMPLE);
        let parser = JuliaParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.jl", SAMPLE);
        let b = write_file(dir.path(), "b.jl", SAMPLE);
        let parser = JuliaParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.jl", SAMPLE);
        let missing = dir.path().join("missing.jl");
        let parser = JuliaParser::new();
        let mut g = graph();
        let project = parser
