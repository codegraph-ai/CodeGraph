...
//! Implementation of the CodeParser trait for Perl
...
/// Perl language parser implementing the CodeParser trait
...
impl CodeParser for PerlParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = PerlParser::new();
        assert_eq!(parser.file_extensions(), &[".pl", ".pm", ".t"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = PerlParser::new();
        assert!(parser.can_parse(Path::new("script.pl")));
        assert!(parser.can_parse(Path::new("Module.pm")));
        assert!(parser.can_parse(Path::new("test.t")));
        assert!(!parser.can_parse(Path::new("main.py")));
        assert!(!parser.can_parse(Path::new("main.rb")));
    }

    use std::io::Write;

    /// A small but syntactically complete Perl source touching every extracted
    /// entity kind: one non-pragma `use` import, one `package` (mapped to a
    /// class), and one `sub` (a function). Perl has no trait concept - the
    /// visitor carries functions, classes, imports, and calls only. Package
    /// scope extends to the next package declaration, so the `sub` is visited at
    /// top level and lands in ir.functions (never nested into class.methods),
    /// pinning functions=1 / classes=1 / traits=0 / imports=1 with
    /// entity_count=2 (functions + classes; entity_count excludes imports).
    /// Pragmas like `strict`, `warnings`, `base`, `parent` are filtered out, so
    /// only `POSIX` counts as an import.
    const SAMPLE: &str = r#"use POSIX qw(floor ceil);
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = PerlParser::default().metrics();
        let new = PerlParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = PerlParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("User.pm"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one sub");
        assert_eq!(info.classes.len(), 1, "one package");
        assert_eq!(info.traits.len(), 0, "Perl has no traits");
        assert_eq!(info.imports.len(), 1, "one non-pragma use");
        assert_eq!(
            info.entity_count(),
            2,
            "functions + classes (imports excluded)"
        );
    }
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Perl is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = PerlParser::new();
        let mut g = graph();
        let src = "# just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.pl"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "User.pm", SAMPLE);
        let parser = PerlParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.pl"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.pl", SAMPLE);
        let parser = PerlParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.pl", SAMPLE);
        let b = write_file(dir.path(), "b.pl", SAMPLE);
        let parser = PerlParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.pl", SAMPLE);
        let b = write_file(dir.path(), "b.pl", SAMPLE);
        let parser = PerlParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.pl", SAMPLE);
