...
//! Integration tests for Zig parser
...
const SAMPLE_APP: &str = include_str!("fixtures/sample_app.zig");

#[test]
fn test_parse_sample_app_structs() {
    let parser = ZigParser::new();
    let mut graph = CodeGraph::in_memory().unwrap();

    let file_info = parser
        .parse_source(SAMPLE_APP, Path::new("sample_app.zig"), &mut graph)
        .unwrap();

    // Should find Entity, User, Product structs
    assert!(
        file_info.classes.len() >= 3,
        "Expected at least 3 structs, found {}",
    ...
    assert!(
        struct_names.iter().any(|n| n.contains("Entity")),
        "Should contain Entity struct, found: {:?}",
        struct_names
    );
    assert!(
        struct_names.iter().any(|n| n.contains("User")),
        "Should contain User struct, found: {:?}",
        struct_names
    );
    assert!(
        struct_names.iter().any(|n| n.contains("Product")),
        "Should contain Product struct, found: {:?}",
        struct_names
    );

    println!("Structs found: {:?}", struct_names);
}
...
fn test_parse_sample_app_functions() {
    ...
    // Should find multiple functions (display, init, addRole, isAdmin, discountedPrice, calculateDiscount)
    assert!(
    ...
    assert!(
        func_names.iter().any(|n| n.contains("isAdmin")),
        "Should contain isAdmin function, found: {:?}",
        func_names
    );
    assert!(
        func_names.iter().any(|n| n.contains("calculateDiscount")),
        "Should contain calculateDiscount function, found: {:?}",
        func_names
    );
    ...
    println!(
        "Sample functions: {:?}",
        &func_names[..func_names.len().min(10)]
    );
}
...
fn test_parse_sample_app_complexity() {
    ...
    // At least one function should have complexity > 1 (addRole, isAdmin have if/for)
...
fn test_parse_sample_app_summary() {
    ...
    println!("\n=== Zig Parser Sample App Summary ===");
    println!("File: sample_app.zig");
    println!("Lines: {}", file_info.line_count);
    println!("Structs (classes): {}", file_info.classes.len());
