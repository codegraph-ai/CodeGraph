...
//! Implementation of the CodeParser trait for Erlang
...
/// Erlang language parser implementing the CodeParser trait
...
impl CodeParser for ErlangParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = ErlangParser::new();
        assert_eq!(parser.file_extensions(), &[".erl", ".hrl"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = ErlangParser::new();
        assert!(parser.can_parse(Path::new("server.erl")));
        assert!(parser.can_parse(Path::new("records.hrl")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Erlang module touching every extracted
    /// entity kind: one `-behaviour` (trait), one `-record` (class), one
    /// `-import` (import) and one function clause. Erlang is a full-OO-shape
    /// parser (functions + records-as-classes + behaviours-as-traits + imports),
    /// so this pins functions=1/classes=1/traits=1/imports=1 with entity_count=3.
    const SAMPLE: &str = r#"-module(greeter).
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = ErlangParser::default().metrics();
        let new = ErlangParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = ErlangParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("greeter.erl"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one function clause");
        assert_eq!(info.classes.len(), 1, "one record as class");
        assert_eq!(info.traits.len(), 1, "one behaviour as trait");
        assert_eq!(info.imports.len(), 1, "one import attribute");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Erlang is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = ErlangParser::new();
        let mut g = graph();
        let src = "% just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.erl"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "greeter.erl", SAMPLE);
        let parser = ErlangParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.erl"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.erl", SAMPLE);
        let parser = ErlangParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_reset_metrics_zeroes_state() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "greeter.erl", SAMPLE);
        let mut parser = ErlangParser::new();
        let mut g = graph();
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.erl", SAMPLE);
        let b = write_file(dir.path(), "b.erl", SAMPLE);
        let parser = ErlangParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.erl", SAMPLE);
        let b = write_file(dir.path(), "b.erl", SAMPLE);
        let parser = ErlangParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.erl", SAMPLE);
        let missing = dir.path().join("missing.erl");
        let parser = ErlangParser::new();
        let mut g = graph();
        let project = parser
