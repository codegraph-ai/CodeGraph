...
//! Implementation of the CodeParser trait for Dart
...
/// Dart language parser implementing the CodeParser trait
...
impl CodeParser for DartParser {
...
mod tests {
    ...
    fn test_can_parse() {
        let parser = DartParser::new();
        assert!(parser.can_parse(Path::new("app.dart")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Dart source touching every extracted
    /// entity kind: one import, one mixin (mapped to a trait), one class, one
    /// top-level function. Both the mixin and class are kept method-free so the
    /// visitor's recursion into their bodies cannot inflate the function count
    /// past the single top-level function.
    const SAMPLE: &str = "import 'dart:math';\n\nmixin Walkable {}\n\nclass Person {\n  String name;\n  int age;\n}\n\nint add(int a, int b) {\n  return a + b;\n}\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = DartParser::default().metrics();
        let new = DartParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = DartParser::with_config(cfg);
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Dart is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = DartParser::new();
        let mut g = graph();
        let src = "// just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.dart"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.dart", SAMPLE);
        let parser = DartParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.dart"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.dart", SAMPLE);
        let parser = DartParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.dart", SAMPLE);
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.dart", SAMPLE);
        let b = write_file(dir.path(), "b.dart", SAMPLE);
        let parser = DartParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.dart", SAMPLE);
