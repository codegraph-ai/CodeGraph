...
//! Implementation of the CodeParser trait for Scala
...
/// Scala language parser implementing the CodeParser trait
...
impl CodeParser for ScalaParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = ScalaParser::new();
        assert_eq!(parser.file_extensions(), &[".scala", ".sc"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = ScalaParser::new();
        assert!(parser.can_parse(Path::new("Main.scala")));
        assert!(parser.can_parse(Path::new("script.sc")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Scala source touching every extracted
    /// entity kind: one import, one class, one (body-less) trait, one top-level
    /// function. The class and trait are kept method-free so the visitor's
    /// recursion into their bodies cannot inflate the function count past one.
    const SAMPLE: &str = "import scala.collection.mutable.ListBuffer\n\ntrait Walkable\n\nclass Person(val name: String, val age: Int)\n\ndef add(a: Int, b: Int): Int = a + b\n";
    ...
    fn test_default_matches_new_metrics() {
        let default = ScalaParser::default().metrics();
        let new = ScalaParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = ScalaParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("lib.scala"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one top-level function");
        assert_eq!(info.classes.len(), 1, "class maps to a class");
        assert_eq!(info.traits.len(), 1, "trait maps to a trait");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Scala is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = ScalaParser::new();
        let mut g = graph();
        let src = "// just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.scala"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.scala", SAMPLE);
        let parser = ScalaParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.scala"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.scala", SAMPLE);
        let parser = ScalaParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_reset_metrics_zeroes_state() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "lib.scala", SAMPLE);
        let mut parser = ScalaParser::new();
        let mut g = graph();
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.scala", SAMPLE);
        let b = write_file(dir.path(), "b.scala", SAMPLE);
        let parser = ScalaParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.scala", SAMPLE);
        let b = write_file(dir.path(), "b.scala", SAMPLE);
        let parser = ScalaParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.scala", SAMPLE);
        let missing = dir.path().join("missing.scala");
        let parser = ScalaParser::new();
        let mut g = graph();
        let project = parser
