...
//! Implementation of the CodeParser trait for HCL/Terraform
...
/// HCL/Terraform language parser implementing the CodeParser trait
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = HclParser::new();
        assert_eq!(parser.file_extensions(), &[".tf", ".hcl", ".tfvars"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = HclParser::new();
        assert!(parser.can_parse(Path::new("main.tf")));
        assert!(parser.can_parse(Path::new("config.hcl")));
        assert!(parser.can_parse(Path::new("vars.tfvars")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete HCL/Terraform config touching every
    /// extracted entity kind: one `module` block (import) and one `resource`
    /// block (function). HCL has no class or trait concept - the visitor carries
    /// only functions, imports, and calls - so this pins functions=1/classes=0/
    /// traits=0/imports=1 with entity_count=1.
    const SAMPLE: &str = r#"module "vpc" {
  source = "./modules/vpc"
}
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = HclParser::default().metrics();
        let new = HclParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = HclParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("main.tf"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one resource block");
        assert_eq!(info.classes.len(), 0, "HCL has no classes");
        assert_eq!(info.traits.len(), 0, "HCL has no traits");
        assert_eq!(info.imports.len(), 1, "one module block");
        assert_eq!(info.entity_count(), 1, "functions only");
    }
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // HCL is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = HclParser::new();
        let mut g = graph();
        let src = "# just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.tf"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "main.tf", SAMPLE);
        let parser = HclParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.tf"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.tf", SAMPLE);
        let parser = HclParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.tf", SAMPLE);
        let b = write_file(dir.path(), "b.tf", SAMPLE);
        let parser = HclParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.tf", SAMPLE);
        let b = write_file(dir.path(), "b.tf", SAMPLE);
        let parser = HclParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.tf", SAMPLE);
        let missing = dir.path().join("missing.tf");
        let parser = HclParser::new();
        let mut g = graph();
        let project = parser
