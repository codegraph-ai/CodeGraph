...
mod tests {
    ...
    /// A small but syntactically complete C# source touching every extracted
    /// entity kind: one using (import), one interface (trait) with one required
    /// method, one class with one method. The C# visitor pushes interface
    /// method signatures into functions (like Java), so the free-standing
    /// function count is exactly two.
    ...
    fn test_default_matches_new_metrics() {
        let default = CSharpParser::default().metrics();
        let new = CSharpParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = CSharpParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("lib.cs"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 2, "interface method + class method");
        assert_eq!(info.classes.len(), 1, "one class");
        assert_eq!(info.traits.len(), 1, "interface maps to a trait");
        assert_eq!(info.imports.len(), 1, "one using");
        assert_eq!(info.entity_count(), 4, "functions + classes + traits");
    }
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        ...
        let info = parser
            .parse_source(src, Path::new("empty.cs"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "lib.cs", SAMPLE);
        let parser = CSharpParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.cs"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.cs", SAMPLE);
        let parser = CSharpParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.cs", SAMPLE);
        let b = write_file(dir.path(), "b.cs", SAMPLE);
        let parser = CSharpParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.cs", SAMPLE);
        let b = write_file(dir.path(), "b.cs", SAMPLE);
        let parser = CSharpParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.cs", SAMPLE);
