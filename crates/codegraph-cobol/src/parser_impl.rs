...
mod tests {
    ...
    /// A small, complete COBOL program touching each entity kind COBOL supports.
    /// The `program-id. SAMPLE` yields one class, the `copy MYBOOK` yields one
    /// import, and the `MAIN-PARA` paragraph yields one function. COBOL has no
    /// trait concept, so this pins functions=1 / classes=1 / traits=0 / imports=1
    /// with entity_count=2 (functions + classes + traits, which excludes imports).
    ...
    fn test_default_matches_new_metrics() {
        let default = CobolParser::default().metrics();
        let new = CobolParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = CobolParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("sample.cob"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one paragraph");
        assert_eq!(info.classes.len(), 1, "one program");
        assert_eq!(info.traits.len(), 0, "COBOL has no traits");
        assert_eq!(info.imports.len(), 1, "one COPY import");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // COBOL is tree-sitter-based and error-tolerant, so a comment-only
        // source (an asterisk in the indicator column) parses and extracts
        // no entities: without a program-id there is no class.
        let parser = CobolParser::new();
        let mut g = graph();
        let src = "      * just a comment\n";
        let info = parser
            .parse_source(src, Path::new("sample.cob"), &mut g)
    ...
    fn test_parse_source_does_not_touch_metrics() {
        // COBOL's parse_source is metric-free; only parse_file records metrics.
        let parser = CobolParser::new();
        let mut g = graph();
        parser
            .parse_source(SAMPLE, Path::new("sample.cob"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "sample.cob", SAMPLE);
        let parser = CobolParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/sample.cob"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "sample.cob", SAMPLE);
        let parser = CobolParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.cob", SAMPLE);
        let b = write_file(dir.path(), "b.cob", SAMPLE);
        let parser = CobolParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.cob", SAMPLE);
        let missing = dir.path().join("missing.cob");
        let parser = CobolParser::new();
        let mut g = graph();
        let project = parser
