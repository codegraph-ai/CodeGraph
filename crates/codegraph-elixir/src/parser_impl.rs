...
//! Implementation of the CodeParser trait for Elixir
...
/// Elixir language parser implementing the CodeParser trait
...
impl CodeParser for ElixirParser {
...
mod tests {
    ...
    fn test_file_extensions() {
        let parser = ElixirParser::new();
        assert_eq!(parser.file_extensions(), &[".ex", ".exs"]);
    }

    #[test]
    fn test_can_parse() {
        let parser = ElixirParser::new();
        assert!(parser.can_parse(Path::new("user.ex")));
        assert!(parser.can_parse(Path::new("mix.exs")));
        assert!(!parser.can_parse(Path::new("main.py")));
    }

    use std::io::Write;

    /// A small but syntactically complete Elixir module touching every extracted
    /// entity kind: one `import` directive (import) and one `def` function.
    /// Elixir has no class or trait concept - `defmodule` merely recurses into
    /// its body for functions - so this pins functions=1/classes=0/traits=0/
    /// imports=1 with entity_count=1.
    const SAMPLE: &str = r#"defmodule Greeter do
  import Ecto.Query
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = ElixirParser::default().metrics();
        let new = ElixirParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = ElixirParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("greeter.ex"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 1, "one def function");
        assert_eq!(info.classes.len(), 0, "Elixir has no classes");
        assert_eq!(info.traits.len(), 0, "Elixir has no traits");
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // Elixir is tree-sitter-based and error-tolerant, so a comment-only
        // source parses cleanly and simply extracts nothing.
        let parser = ElixirParser::new();
        let mut g = graph();
        let src = "# just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.ex"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "greeter.ex", SAMPLE);
        let parser = ElixirParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.ex"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.ex", SAMPLE);
        let parser = ElixirParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.ex", SAMPLE);
        let b = write_file(dir.path(), "b.ex", SAMPLE);
        let parser = ElixirParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.ex", SAMPLE);
        let b = write_file(dir.path(), "b.ex", SAMPLE);
        let parser = ElixirParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.ex", SAMPLE);
        let missing = dir.path().join("missing.ex");
        let parser = ElixirParser::new();
        let mut g = graph();
        let project = parser
