...
mod tests {
    ...
    fn build(ir: &CodeIR) -> (CodeGraph, FileInfo) {
        ...
        let info = ir_to_graph(ir, &mut graph, Path::new("Solver.ex")).unwrap();
        (graph, info)
    }
    ...
    fn empty_ir_creates_file_node_from_path_stem() {
        let ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        ...
        assert_eq!(
            ...
            Some(&PropertyValue::String("elixir".to_string()))
        );
    ...
    fn module_drives_file_node_metadata_and_line_count() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        let mut module = ModuleEntity::new("Solver", "lib/solver.ex", "elixir");
        ...
        assert_eq!(
            ...
            Some(&PropertyValue::String("lib/solver.ex".to_string()))
        );
        assert_eq!(
    ...
    fn classes_are_ignored_by_the_mapper() {
        ...
        let (graph, info) = build(&ir);
        // The elixir mapper never iterates ir.classes, so nothing is emitted.
        assert!(info.classes.is_empty());
        assert!(info.functions.is_empty());
        assert_eq!(graph.node_count(), 1);
    }
    ...
    fn traits_are_ignored_by_the_mapper() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        ir.add_trait(TraitEntity::new("Enumerable", 1, 3));

        let (graph, info) = build(&ir);
        // The elixir mapper never iterates ir.traits, so no Interface node exists.
        assert!(info.traits.is_empty());
        assert_eq!(graph.node_count(), 1);
        assert!(graph
            .nodes_iter()
    ...
    fn free_function_is_contained_by_file_with_complexity_and_flag_props() {
        ...
        // Elixir keeps function names bare (no Class#/Class. qualification).
    ...
    fn import_creates_external_module_with_empty_edge_props() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        ir.add_import(ImportRelation::new("Solver", "Enum").with_symbols(vec!["map".to_string()]));
        ...
        // The elixir mapper records NO props on the Imports edge (symbols dropped).
    ...
    fn duplicate_import_target_reuses_existing_node() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        ir.add_import(ImportRelation::new("Solver", "Kernel"));
        ir.add_import(ImportRelation::new("Solver", "Kernel"));
    ...
    fn function_optional_props_present() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        let func = FunctionEntity::new("solve", 1, 10)
            .with_doc("solves it")
            .with_body_prefix("def solve(x) do")
        ...
        assert_eq!(
            ...
            Some(PropertyValue::String("def solve(x) do".to_string()))
        );
        assert_eq!(
    ...
    fn function_optional_props_absent_and_unread_fields_never_stamped() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        // return_type and attributes are set on the entity but the elixir
        // mapper never reads them onto the node.
        let func = FunctionEntity::new("solve", 1, 10)
            .with_return_type("integer")
            .with_attributes(vec!["@spec".to_string()]);
        ir.add_function(func);
    ...
    fn function_all_complexity_sub_props_stamped_with_grade() {
        ...
        ir.add_function(FunctionEntity::new("solve", 1, 40).with_complexity(metrics));
    ...
    fn function_without_complexity_omits_all_complexity_props() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        ir.add_function(FunctionEntity::new("solve", 1, 10));
    ...
    fn function_boolean_flags_stamped() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        let mut func = FunctionEntity::new("solve", 1, 10);
        func.is_async = true;
        func.is_static = true;
    ...
    fn function_signature_visibility_and_line_bounds_stamped() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        ir.add_function(
            FunctionEntity::new("solve", 7, 21)
                .with_signature("defp solve(x)")
        ...
        assert_eq!(
            ...
            Some(PropertyValue::String("defp solve(x)".to_string()))
        );
        assert_eq!(
    ...
    fn module_without_doc_omits_doc_prop() {
        let mut ir = CodeIR::new(std::path::PathBuf::from("Solver.ex"));
        let module = ModuleEntity::new("Solver", "lib/solver.ex", "elixir");
        ir.set_module(module);
