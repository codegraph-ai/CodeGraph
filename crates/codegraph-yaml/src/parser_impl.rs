...
//! Implementation of the CodeParser trait for YAML
...
/// YAML language parser implementing the CodeParser trait
...
impl CodeParser for YamlParser {
...
mod tests {
    ...
    fn test_language() {
        ...
        assert_eq!(parser.language(), "yaml");
    }

    #[test]
    fn test_file_extensions() {
        let parser = YamlParser::new();
        assert_eq!(parser.file_extensions(), &[".yml", ".yaml"]);
    }
    ...
    /// A small YAML document exercising the only entity kind YAML extracts:
    /// top-level mapping keys, each mapped to an ir.function. YAML never carries
    /// classes, traits, or imports (the extractor clears imports/calls and the
    /// mapper ignores everything but functions), so nested keys under `metadata`
    /// are NOT counted - only the three top-level keys apiVersion/kind/metadata
    /// become functions. This pins functions=3/classes=0/traits=0/imports=0 with
    /// entity_count=3.
    const SAMPLE: &str = r#"apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
  labels:
...
"#;
    ...
    fn test_default_matches_new_metrics() {
        let default = YamlParser::default().metrics();
        let new = YamlParser::new().metrics();
    ...
    fn test_with_config_and_accessor() {
        ...
        let parser = YamlParser::with_config(cfg);
    ...
    fn test_parse_source_extracts_each_entity_kind() {
        ...
        let mut g = graph();
        let info = parser
            .parse_source(SAMPLE, Path::new("deploy.yaml"), &mut g)
            .expect("parse ok");
        assert_eq!(info.functions.len(), 3, "three top-level keys");
        assert_eq!(info.classes.len(), 0, "yaml has no classes");
        assert_eq!(info.traits.len(), 0, "yaml has no traits");
        assert_eq!(info.imports.len(), 0, "yaml has no imports");
        assert_eq!(info.entity_count(), 3, "functions only");
    }
    ...
    fn test_parse_source_comment_only_yields_no_entities() {
        // YAML comments use `#`; a comment-only source parses cleanly and
        // extracts nothing.
        let parser = YamlParser::new();
        let mut g = graph();
        let src = "# just a comment\n";
        let info = parser
            .parse_source(src, Path::new("empty.yaml"), &mut g)
    ...
    fn test_parse_file_success_updates_metrics() {
        ...
        let path = write_file(dir.path(), "deploy.yaml", SAMPLE);
        let parser = YamlParser::new();
        let mut g = graph();
    ...
    fn test_parse_file_missing_file_is_io_error() {
        ...
        let mut g = graph();
        let err = parser
            .parse_file(Path::new("/no/such/file.yaml"), &mut g)
    ...
    fn test_parse_file_too_large() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_file(dir.path(), "big.yaml", SAMPLE);
        let parser = YamlParser::with_config(ParserConfig::default().with_max_file_size(4));
        let mut g = graph();
        let err = parser
    ...
    fn test_metrics_accumulate_across_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let a = write_file(dir.path(), "a.yaml", SAMPLE);
        let b = write_file(dir.path(), "b.yaml", SAMPLE);
        let parser = YamlParser::new();
        let mut g = graph();
    ...
    fn test_parse_files_sequential_success() {
        ...
        let a = write_file(dir.path(), "a.yaml", SAMPLE);
        let b = write_file(dir.path(), "b.yaml", SAMPLE);
        let parser = YamlParser::new(); // parallel = false by default
        let mut g = graph();
    ...
    fn test_parse_files_partitions_failures() {
        let dir = tempfile::tempdir().expect("tempdir");
        let good = write_file(dir.path(), "good.yaml", SAMPLE);
